...
window.injectBookTransition = function () {
    ...
    loader.innerHTML = `
        ...
        <div style="position:absolute; font-family:'Tajawal'; color:#fff; font-size:40px; font-weight:900; z-index:2; transition: opacity 0.5s;" class="book-logo">علاء الدين</div>
    `;
...
window.injectCountryModal = function () {
    ...
    modal.innerHTML = `
        <div style="background:#fff; width:90%; max-width:600px; border-radius:30px; padding:40px; position:relative; box-shadow:0 30px 60px rgba(0,0,0,0.5); text-align:center;">
            ...
            <h2 style="font-size:32px; font-weight:900; color:#132644; margin-bottom:30px;">اختر الدولة <span style="color:#12b8c5;">علاء الدين</span></h2>
...
window.sendWhatsApp = function (msg) {
    let finalMsg = "أهلاً بك في منصة علاء الدين التعليمية.\n\n" + msg;
...
window.requestCode = function () {
    let msg = "أريد طلب كود اشتراك جديد لمنصة علاء الدين التعليمية";
    ...
    if (!document.getElementById('my-subs-login-modal')) {
        ...
        modal.innerHTML = `
            <div style="background:#fff; border-radius:20px; padding:30px; width:90%; max-width:500px; text-align:center; box-shadow:0 20px 50px rgba(0,0,0,0.2);">
                ...
                <a href="#" onclick="window.sendWhatsApp('أهلاً، نسيت كود الاشتراك الخاص بي لمنصة علاء الدين، هل يمكنكم مساعدتي؟'); return false;" style="color:#f44336; font-weight:bold; text-decoration:underline;">هل نسيت الكود؟</a>
...
window.registerAttendance = async function () {
    ...
    alert("ممتاز! تم تسجيل حضورك بنجاح في سجلات منصة علاء الدين للإدارة 🎓");
}
...
window.promptAdmin = async function () {
    let pass = prompt("بوابة علاء الدين للإدارة - يرجى كتابة الرقم السري (الأساسي أو المساعد):");
...
window.injectWhatsAppButton = function () {
    ...
    btn.href = 'https://wa.me/201123511459?text=' + encodeURIComponent('أهلاً بك في منصة علاء الدين التعليمية.\\n');
    btn.target = '_blank';
